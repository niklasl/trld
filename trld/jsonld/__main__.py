...
for doc_path in args.source:
    ...
    try:
        if context_data:
            doc_data = expand(doc_data, base_iri, ordered=ordered)
            result = compact(context_data, doc_data, base_iri, ordered=ordered)
        else:
            result = expand(doc_data, base_iri, ordered=ordered)
